  ...
  return (
    <section id="angebot" className="scroll-mt-24 bg-cream py-24 md:py-36">
      <div className="mx-auto max-w-[1320px] px-6 md:px-10">
        ...
        <div className="mt-14 grid gap-px overflow-hidden rounded-[4px] border border-greige/25 bg-greige/20 md:mt-20 md:grid-cols-2">
          {o.items.map((it, i) => (
            <Reveal key={it.tag} delay={i * 0.06} className="h-full">
              <div className="flex h-full flex-col bg-ivory p-8 md:p-12">
                <span className="font-mono text-[0.7rem] tracking-[0.2em] text-clay">
                  {it.tag}
                </span>
                <h3 className="mt-5 font-display text-[1.8rem] font-light text-espresso">
                  {it.title}
                </h3>
                <p className="mt-3 max-w-sm text-[0.94rem] font-light leading-relaxed text-mushroom">
                  {it.text}
                </p>
              </div>
            </Reveal>
